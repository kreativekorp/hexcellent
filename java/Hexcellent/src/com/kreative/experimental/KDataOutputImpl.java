...
public final class KDataOutputImpl implements DataOutput, KDataOutput {
	...
	public final void writeSignedBEB128(BigInteger v) throws IOException { writeBEB128(v, true); }
	public final void writeUnsignedBEB128(BigInteger v) throws IOException { writeBEB128(v, false); }
	private final void writeBEB128(BigInteger v, boolean signed) throws IOException {
	...
	public final void writeSignedLEB128(BigInteger v) throws IOException { writeLEB128(v, true); }
	public final void writeUnsignedLEB128(BigInteger v) throws IOException { writeLEB128(v, false); }
	private final void writeLEB128(BigInteger v, boolean signed) throws IOException {
		int n = (v.bitLength() + (signed ? 7 : 6)) / 7;
		while (n > 1) {
			n--;
	...
	private final void writeFP128NaN(boolean le, boolean negative, boolean quiet, BigInteger pattern) throws IOException {
		byte header = 0x40;
	...
	private final void writeFP128NaN(boolean le, FloatFormat.NaN nan) throws IOException {
		writeFP128NaN(le, nan.isNegative(), nan.isQuiet(), nan.getPattern());
	}
	
	private final void writeFP128Zero(boolean negative) throws IOException {
		out.writeByte(negative ? 0x80 : 0x00);
	}
	
	private final void writeFP128Zero(FloatFormat.Zero zero) throws IOException {
		writeFP128Zero(zero.isNegative());
	}
	
	private final void writeFP128Finite(boolean le, boolean decimal, boolean negative, BigInteger exponent, BigInteger mantissa) throws IOException {
	...
	private final void writeFP128BigDecimal(boolean le, BigDecimal value) throws IOException {
		if (value.signum() == 0) {
	...
	private final void writeFP128BigInteger(boolean le, BigInteger value) throws IOException {
		if (value.signum() == 0) {
			writeFP128Zero(false);
			return;
		}
	...
	private final void writeFP128Long(boolean le, long value) throws IOException {
		if (value == 0) {
			writeFP128Zero(false);
			return;
		}
	...
	private final void writeFP128Int(boolean le, int value) throws IOException {
		if (value == 0) {
			writeFP128Zero(false);
			return;
		}
	...
	private final void writeFP128Float(boolean le, float value) throws IOException {
	...
	private final void writeFP128Double(boolean le, double value) throws IOException {
	...
	public final void writeLEFP128(Number value) throws IOException { writeFP128(true, value); }
	public final void writeBEFP128(Number value) throws IOException { writeFP128(false, value); }
	private final void writeFP128(boolean le, Number value) throws IOException {
