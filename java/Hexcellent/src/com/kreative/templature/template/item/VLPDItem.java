...
public final class VLPDItem implements TemplateItem {
	...
	public VLPDItem(int typeConstant, String typeString, String name, boolean le, int radix) {
	...
	public Instance read(Closure closure, BufferedCountedInputStream in) throws IOException {
		if (le) return new Instance(closure, VLFPItem.readLEB100(in));
		else    return new Instance(closure, VLFPItem.readBEB100(in));
	}
	...
	public final class Instance implements TemplateItemInstance, ClosureItem, StringDataModel {
		...
		public void write(BufferedCountedOutputStream out) throws IOException {
			if (le) VLFPItem.writeLEB100(out, value);
			else    VLFPItem.writeBEB100(out, value);
		}
	}
}
