...
public final class PackedFloatItem implements PackedItem {
	...
	public PackedFloatItem(int typeConstant, String typeString, String name, FloatFormat format) {
		this.typeConstant = typeConstant;
		this.typeString = typeString;
		this.name = name;
		this.format = format;
		this.width = format.getExponentWidth() + format.getMantissaWidth() + 1;
	...
	public Instance read(Closure closure, BigIntegerBitInputStream in) {
		return new Instance(closure, format.bitsToNumber(in.read(width, false)));
	}
	...
	public final class Instance implements PackedItemInstance, ClosureItem, StringDataModel {
		...
		public void write(BigIntegerBitOutputStream out) {
			out.write(format.numberToBits(value), width);
		}
	}
}
