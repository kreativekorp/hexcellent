...
public final class PackedIntegerItem implements PackedItem {
	...
	public PackedIntegerItem(int typeConstant, String typeString, String name, int width, boolean signed, int radix) {
	...
	public Instance read(Closure closure, BigIntegerBitInputStream in) {
		return new Instance(closure, in.read(width, signed));
	}
	...
	public final class Instance implements PackedItemInstance, ClosureItem, StringDataModel {
		...
		public String getStringValue() {
			return TemplateUtils.formatBigInteger(value, width, radix);
		}
		...
		public <C> C createComponent(ComponentFactory<C> factory) {
			BigInteger max = BigInteger.valueOf(-1).shiftLeft(width).not();
			int columns = TemplateUtils.formatBigInteger(max, width, radix).length() + 2;
