...
public final class FloatFormat implements Serializable {
	...
	public int getExponentWidth() { return expWidth; }
	public int getMantissaWidth() { return manWidth; }
	public int getExponentBias() { return expBias.intValue(); }
	...
	public BigInteger nanToBits(boolean negative, boolean quiet, BigInteger pattern) {
		...
		if (quiet) bits = bits.setBit(quietBit);
		bits = bits.or(expMask.shiftLeft(manWidth));
		if (negative) bits = bits.setBit(signBit);
		return bits;
	}
	...
	public BigInteger binaryToBits(boolean negative, BigInteger exponent, BigInteger mantissa) {
		...
		if (sign == 0) return zeroToBits(negative);
		if (sign < 0) {
			negative = !negative;
	...
	public BigInteger floatToBits(float value) {
		...
		if (rawExp == 0) {
			...
			if (rawMan == 0) return zeroToBits(negative);
			mantissa = rawMan << 1;
		} else {
	...
	public BigInteger numberToBits(Number value) {
		if (value instanceof NaN) return nanToBits((NaN)value);
		if (value instanceof Zero) return zeroToBits((Zero)value);
		if (value instanceof BigDecimal) return bigDecimalToBits((BigDecimal)value);
		if (value instanceof BigInteger) return bigIntegerToBits((BigInteger)value);
		if (value instanceof Long) return longToBits(value.longValue());
		if (value instanceof Integer) return intToBits(value.intValue());
		if (value instanceof Short) return intToBits(value.intValue());
		if (value instanceof Byte) return intToBits(value.intValue());
		if (value instanceof Float) return floatToBits(value.floatValue());
		if (value instanceof Double) return doubleToBits(value.doubleValue());
	...
	public static final class Zero extends Number {
		...
		private Zero(boolean n) { this.n = n; }
		public boolean isNegative() { return n; }
		
		public Zero abs() { return POSITIVE_ZERO; }
		public Zero negate() { return n ? POSITIVE_ZERO : NEGATIVE_ZERO; }
		public Zero plus() { return n ? NEGATIVE_ZERO : POSITIVE_ZERO; }
		public int signum() { return 0; }
		
		@Override public float floatValue() { return Float.intBitsToFloat(n ? FV : 0); }
		@Override public double doubleValue() { return Double.longBitsToDouble(n ? DV : 0); }
		@Override public byte byteValue() { return 0; }
		@Override public short shortValue() { return 0; }
		@Override public int intValue() { return 0; }
		@Override public long longValue() { return 0; }
		
		@Override public boolean equals(Object o) { return o instanceof Zero && n == ((Zero)o).n; }
		@Override public int hashCode() { return n ? FV : 0; }
		@Override public String toString() { return n ? "-0.0" : "0.0"; }
	}
	...
	public static final class NaN extends Number {
		...
		public NaN(boolean negative, boolean quiet, BigInteger pattern) {
			if (pattern == null) pattern = BigInteger.ZERO;
		...
		public NaN(float value) {
			...
			if ((bits & FLOAT_I) != FLOAT_I) throw new IllegalArgumentException("Not a NaN");
		...
		public NaN(double value) {
			...
			if ((bits & DOUBLE_I) != DOUBLE_I) throw new IllegalArgumentException("Not a NaN");
		...
		public BigInteger getPattern() { return pattern; }
		
		public NaN abs() { return new NaN(false, quiet, pattern); }
		public NaN negate() { return new NaN(!negative, quiet, pattern); }
		public NaN plus() { return new NaN(!!negative, quiet, pattern); }
		public int signum() {
			if (quiet || !pattern.equals(BigInteger.ZERO)) return 0;
			else return (negative ? -1 : 1);
		}
		
		@Override
		public float floatValue() {
			int bits = (pattern.intValue() & FLOAT_P) | FLOAT_I;
			if (negative) bits |= FLOAT_S;
			if (quiet) bits |= FLOAT_Q;
		...
		public double doubleValue() {
			...
			if (negative) bits |= DOUBLE_S;
			if (quiet) bits |= DOUBLE_Q;
		...
		public int hashCode() {
			...
			if (negative) hash ^= 0x80000000;
			if (quiet) hash ^= 0x40000000;
			return hash;
		}
		
		@Override
		public String toString() {
			if (quiet || !pattern.equals(BigInteger.ZERO)) return "NaN";
			else return (negative ? "-Infinity" : "Infinity");
		}
	}
}
