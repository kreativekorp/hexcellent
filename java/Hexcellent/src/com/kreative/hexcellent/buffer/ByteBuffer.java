...
public abstract class ByteBuffer {
	...
	public final long indexOf(byte[] pattern, long index) {
		if (pattern.length == 0) return index;
		int[] shift = new int[256];
		for (int i = 0; i < 256; i++) shift[i] = pattern.length;
		for (int i = 0, n = pattern.length - 1; i < n; i++) shift[pattern[i] & 0xFF] = n - i;
		byte[] text = new byte[pattern.length];
		long maxIndex = length() - pattern.length;
		while (index <= maxIndex) {
			get(index, text, 0, pattern.length);
			if (Arrays.equals(text, pattern)) return index;
	...
	public final long lastIndexOf(byte[] pattern, long index) {
		...
		int[] shift = new int[256];
		for (int i = 0; i < 256; i++) shift[i] = pattern.length;
		for (int i = pattern.length - 1; i > 0; i--) shift[pattern[i] & 0xFF] = i;
		byte[] text = new byte[pattern.length];
