...
public class ArrayByteBuffer extends ByteBuffer {
	...
	public ArrayByteBuffer(byte[] src, int offset, int length) {
		this.array = new byte[length];
		for (int i = 0; i < length; i++) this.array[i] = src[offset++];
	}
	...
	public boolean get(long offset, byte[] dst, int dstOffset, int length) {
		...
		for (int i = 0; i < length; i++) dst[dstOffset++] = array[(int)offset++];
		return true;
	}
	...
	public boolean insert(long offset, byte[] src, int srcOffset, int length) {
		...
		if (newLength < 0 || newLength > MAX_ARRAY_LENGTH) return false;
		byte[] newArray = new byte[(int)newLength];
		int ni = 0;
		for (int i = 0; i < offset; i++) newArray[ni++] = array[i];
		for (int i = 0; i < length; i++) newArray[ni++] = src[srcOffset++];
		for (int i = (int)offset; i < array.length; i++) newArray[ni++] = array[i];
		array = newArray;
	...
	public boolean overwrite(long offset, byte[] src, int srcOffset, int length) {
		if (length <= 0) return true;
		for (int i = 0; i < length; i++) array[(int)offset++] = src[srcOffset++];
	...
	public boolean remove(long offset, long length) {
		...
		int ni = 0;
		for (int i = 0; i < offset; i++) newArray[ni++] = array[i];
		for (int i = (int)(offset + length); i < array.length; i++) newArray[ni++] = array[i];
		array = newArray;
