...
public class ByteBufferGetTest {
	...
	private static void testGet() throws IOException {
		...
		randomByteBuffer(start, end).get(0, actual, 0, end - start);
		System.out.print(Arrays.equals(expected, actual) ? "\tPASS" : "\tFAIL\u0007");
		
		actual = new byte[end - start];
		randomByteBuffer(0, 256).get(start, actual, 0, end - start);
		System.out.print(Arrays.equals(expected, actual) ? "\tPASS" : "\tFAIL\u0007");
		...
		randomByteBuffer(start, end).get(0, actual, offset, end - start);
		System.out.print(equals(expected, 0, actual, offset, end - start) ? "\tPASS" : "\tFAIL\u0007");
		
		actual = new byte[end - start + padding];
		randomByteBuffer(0, 256).get(start, actual, offset, end - start);
		System.out.print(equals(expected, 0, actual, offset, end - start) ? "\tPASS" : "\tFAIL\u0007");
