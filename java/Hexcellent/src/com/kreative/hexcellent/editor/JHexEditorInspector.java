...
public class JHexEditorInspector extends JPanel {
	...
	public JHexEditorInspector(JHexEditor parent) {
		...
		new FieldListener(binField) {
			...
			public BigInteger parse(String text, int length) {
				try { return new BigInteger(text.trim(), 2); }
		...
		new FieldListener(octField) {
			@Override
			public BigInteger parse(String text, int length) {
				try { return new BigInteger(text.trim(), 8); }
				catch (Exception e) { return null; }
			}
		};
		new FieldListener(hexField) {
			@Override
			public BigInteger parse(String text, int length) {
				try { return new BigInteger(text.trim(), 16); }
				catch (Exception e) { return null; }
			}
		};
		new FieldListener(signedField) {
			@Override
			public BigInteger parse(String text, int length) {
				try { return new BigInteger(text.trim(), 10); }
				catch (Exception e) { return null; }
			}
		};
		...
		new FieldListener(floatField) {
			@Override
			public BigInteger parse(String text, int length) {
				try { return stringToFloatingPointBits(length, text.trim()); }
				catch (Exception e) { return null; }
			}
		};
	}
	...
	private void clearFields(JTextField src) {
		inputLock = true;
		if (src != binField) binField.setText("");
		if (src != octField) octField.setText("");
		if (src != hexField) hexField.setText("");
		if (src != signedField) signedField.setText("");
		if (src != unsignedField) unsignedField.setText("");
		if (src != fixedField) fixedField.setText("");
		if (src != floatField) floatField.setText("");
		inputLock = false;
	}
	...
	private void setFieldValues(JTextField src, BigInteger value, int length) {
		if (value == null || length <= 0 || length > MAX_LENGTH) {
			...
			inputLock = true;
			if (src != binField) binField.setText(uValue.toString(2));
			if (src != octField) octField.setText(uValue.toString(8));
			if (src != hexField) hexField.setText(uValue.toString(16).toUpperCase());
			if (src != signedField) signedField.setText(sValue.toString());
			if (src != unsignedField) unsignedField.setText(uValue.toString());
			if (src != fixedField) {
				BigDecimal fixedBase = new BigDecimal(BigInteger.ONE.shiftLeft(length * 4));
				BigDecimal fixedValue = new BigDecimal(sValue).divide(fixedBase);
				fixedField.setText(fixedValue.toString());
			}
			if (src != floatField) floatField.setText(floatingPointBitsToString(length, value));
			inputLock = false;
		}
	}
	...
	private void pullFromEditor(JTextField src) {
		...
		if (data == null || data.length <= 0 || data.length > MAX_LENGTH) {
			...
			if (parent.isLittleEndian()) ReverseTransform.BYTES.transform(data, 0, data.length);
	...
	private abstract class FieldListener implements DocumentListener, FocusListener, KeyListener {
		...
		public void changedUpdate(DocumentEvent e) {
			if (inputLock || !isVisible()) return;
			long length = parent.getSelectionLength();
			if (length <= 0 || length > MAX_LENGTH) return;
			BigInteger value = parse(field.getText(), (int)length);
			if (value != null) pushToEditor(field, value);
			changed = true;
		}
		...
		public void focusLost(FocusEvent e) {
			if (inputLock || !isVisible() || !changed) return;
			long length = parent.getSelectionLength();
			if (length <= 0 || length > MAX_LENGTH) return;
			BigInteger value = parse(field.getText(), (int)length);
			if (value == null) pullFromEditor(null);
			else pushToEditor(null, value);
			changed = false;
		}
	...
	private static JPanel gridLayout(int rows, int cols, JPanel... panels) {
		...
		for (JPanel panel : panels) p.add(panel);
		return p;
	}
	...
	private static String floatingPointBitsToString(int bytes, BigInteger bits) {
		switch (bytes) {
			case 1: return Float.toString(FloatFormat.RESPL8.bitsToNumber(bits).floatValue());
			case 2: return Float.toString(FloatFormat.HALF.bitsToNumber(bits).floatValue());
			case 3: return Float.toString(FloatFormat.FP24.bitsToNumber(bits).floatValue());
			case 4: return Float.toString(Float.intBitsToFloat(bits.intValue()));
			case 6: return Double.toString(FloatFormat.RESPL48.bitsToNumber(bits).doubleValue());
			case 8: return Double.toString(Double.longBitsToDouble(bits.longValue()));
			default: return "";
		}
	}
	...
	private static BigInteger stringToFloatingPointBits(int bytes, String s) {
		switch (bytes) {
			case 1: return FloatFormat.RESPL8.floatToBits(Float.parseFloat(s));
			case 2: return FloatFormat.HALF.floatToBits(Float.parseFloat(s));
			case 3: return FloatFormat.FP24.floatToBits(Float.parseFloat(s));
			case 4: return BigInteger.valueOf(Float.floatToRawIntBits(Float.parseFloat(s)));
			case 6: return FloatFormat.RESPL48.doubleToBits(Double.parseDouble(s));
			case 8: return BigInteger.valueOf(Double.doubleToRawLongBits(Double.parseDouble(s)));
			default: return null;
		}
	}
